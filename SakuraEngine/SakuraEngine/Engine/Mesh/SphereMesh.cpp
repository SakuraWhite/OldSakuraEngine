		...
		for (uint32_t j = 0; j<= InHeightSubdivision; ++j)//判断高度轴
 		{
			...
			//计算法线 用的是DX的API 
			XMVECTOR Pos = XMLoadFloat3(&MeshData.VertexData[TopIndex].Position);//首先转一下结构 
			XMStoreFloat3(&MeshData.VertexData[TopIndex].Normal, XMVector3Normalize(Pos));//在顶点生成法线
 		}
	} 
