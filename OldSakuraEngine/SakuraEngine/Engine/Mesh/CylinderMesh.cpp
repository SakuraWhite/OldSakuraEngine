...
CCylinderMesh* CCylinderMesh::CreateMesh(float InTopRadius, float InBottomRadius, float InHeight, uint32_t InAxialSubdivision, uint32_t InHeightSubdivision)
{
	...
	for (uint32_t i = 0; i < InHeightSubdivision; ++i)
	{
		for (uint32_t j = 0; j < InAxialSubdivision; ++j)
		{
			//绘制四边形
			//三角形1
			MeshData.IndexDeta.push_back((i + 1) * VertexCircleNum + j + 1);// (i + 1) * n + j + 1
			MeshData.IndexDeta.push_back((i + 1) * VertexCircleNum + j); //(i + 1) * n + j
			MeshData.IndexDeta.push_back(i * VertexCircleNum + j);  //i * n + j
			//绘制三角形2
			MeshData.IndexDeta.push_back(i * VertexCircleNum + j + 1);//i * n + j + 1
			MeshData.IndexDeta.push_back((i + 1) * VertexCircleNum + j + 1);// (i + 1) * n + j + 1
			MeshData.IndexDeta.push_back(i * VertexCircleNum + j);//i * n + j
	...
	//构建顶部顶点
	{
		uint32_t Index = MeshData.VertexDeta.size();//圆柱体顶部起始点
		...
		//圆柱体顶部的中心点
		MeshData.VertexDeta.push_back(FVertex(XMFLOAT3(0.f, Y, 0.f),XMFLOAT4(Colors::Red)));
	
		//绘制顶部面
		float Cpoint = MeshData.VertexDeta.size() - 1;//绘制顶部圆心中心点
		for (uint32_t i = 0; i < InAxialSubdivision; ++i)
		{
			//绘制三角形
			MeshData.IndexDeta.push_back(Cpoint);
			MeshData.IndexDeta.push_back(Index + i + 1);
			MeshData.IndexDeta.push_back(Index + i);
	...
	//构建底部
	{
		...
		uint32_t Index = MeshData.VertexDeta.size();//圆柱体底部起始点
