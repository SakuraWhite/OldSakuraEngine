...
const float PI = 3.1415926535f; //定义Π 3.14
void CMesh::Init()
{
	//当前屏幕的横纵比 一般是16比9
	...
	XMMATRIX Project = XMMatrixPerspectiveFovLH(
		0.25f * PI, //以弧度为单位的自上而下的视场角。这里是45°
		AspectRatio,//视图or屏幕空间 X:Y 的纵横比
		1.0f,    //到近剪裁平面的距离 必须大于零
		1000.f);  //斤剪裁平面到远剪裁面的距离 
...
//////////////////////////////////////////////////////////////////////////////////
	...
	D3D12_GPU_VIRTUAL_ADDRESS ObAddr = objectConstants.get()->GetBuffer()->GetGPUVirtualAddress();   //拿到常量地址 可以通过GetBuffer获取到GPU的地址
 	
	//增加描述
	D3D12_CONSTANT_BUFFER_VIEW_DESC CBVDesc;
	CBVDesc.BufferLocation = ObAddr;//创建描述前需要获得这个Buffer地址
...
//////////////////////////////////////////////////////////////////////////////////
	...
	IndexSize = InRendingData->IndexDeta.size();//保存一份顶点数量
	//获取模型数据的大小
	VertexSizeInBytes = InRendingData->VertexDeta.size() * VertexStrideInBytes;//拿当前顶点数据的Size去乘以自己作的顶点数据
	...
	GPSDesc.RasterizerState.FillMode = D3D12_FILL_MODE_WIREFRAME;//以现况模式显示
	...
	GPSDesc.SampleDesc.Count = GetEngine()->GetDXGISampleCount();//判断多重采样的数量
	GPSDesc.SampleDesc.Quality = GetEngine()->GetDXGISampleQuality();//判断多重采样的质量
	

	GPSDesc.RTVFormats[0] = GetEngine()->GetBackBufferFormat();   //设置RTV的格式 纹理格式后缓冲区
	GPSDesc.DSVFormat = GetEngine()->GetDepthStencilFormat();//设置DSV  深度模板
	//获取D3D驱动
...
void CMesh::Draw(float DeltaTime)
{
	...
	//绑定渲染流水线上的输入槽，可以在输入装配其阶段传入顶点数据
	GetGraphicsCommandList()->IASetVertexBuffers(
		0, //起始输入槽 0~15 一共16个索引
		1, //与输入槽绑定的数量 顶点缓冲区的数量 设输入槽索引为k 那么我们绑定的缓冲区就是n 那公式就是k+n-1  这里有0个输入槽，那么绑定缓冲区就是n=k+1 为1
		&VBV);//输入顶点Buffer视图


	D3D12_INDEX_BUFFER_VIEW IBV = GetIndexBufferView();//IndexBuffer视图 IndexBufferView简称IBV
